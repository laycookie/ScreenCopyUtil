...
impl WindowState {
    pub fn draw(&mut self, _conn: &Connection, qh: &QueueHandle<Self>) {
        for viewer in &mut self.windows {
            ...
            viewer.screenshot_buffer.attach_to(window.wl_surface());
...
impl CompositorHandler for WindowState {
    ...
    fn frame(&mut self, conn: &Connection, qh: &QueueHandle<Self>, _surface: &WlSurface, _time: u32) {
        self.draw(conn, qh);
    }
}
